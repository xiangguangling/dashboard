...
export default function ScoreDistribution({ data }: { data: DashboardData }) {
  ...
  useEffect(() => {
    ...
    const typeColors: Record<string, string> = { '小学': '#f97316', '初中': '#06b6d4', '九年制': '#10b981' };
    ...
    for (const d of dist) {
      ...
      const rate = +(d.score / 44).toFixed(3);
      for (let i = 0; i < d.count; i++) {
        ...
        scatterPoints[t].push([+(d.score + jx).toFixed(1), +(rate + jy).toFixed(3)]);
      }
    }
    ...
    const scatterOption: echarts.EChartsCoreOption = {
      ...
      title: {
        ...
        left: 'center', top: 8,
        textStyle: { color: '#b0b8c8', fontSize: 11, fontWeight: 'normal' },
      },
      tooltip: {
        trigger: 'item',
      ...
      legend: { data: [...types], bottom: 0, textStyle: { color: '#b0b8c8', fontSize: 10 } },
      grid: { left: '8%', right: '6%', top: '14%', bottom: '12%' },
      xAxis: { type: 'value', name: '学校总分（满分44）', nameTextStyle: { color: '#b0b8c8', fontSize: 10 }, axisLabel: { color: '#b0b8c8', fontSize: 9 }, splitLine: { lineStyle: { color: 'rgba(255,255,255,0.04)' } }, min: 34, max: 44 },
      yAxis: { type: 'value', name: '得分率', nameTextStyle: { color: '#b0b8c8', fontSize: 10 }, axisLabel: { color: '#b0b8c8', fontSize: 9, formatter: (v: number) => (v * 100).toFixed(0) + '%' }, splitLine: { lineStyle: { color: 'rgba(255,255,255,0.04)' } }, min: 0.75, max: 1.02 },
      series: types.map(t => ({
        type: 'scatter',
        id: t,
        name: t,
        ...
        universalTransition: { enabled: true, seriesKey: types },
    ...
    const barOption: echarts.EChartsCoreOption = {
      ...
      legend: { data: [...types], bottom: 0, textStyle: { color: '#b0b8c8', fontSize: 10 } },
      grid: { left: '8%', right: '6%', top: '14%', bottom: '12%' },
      xAxis: { type: 'category', data: types, axisLabel: { color: '#eef0f4', fontSize: 12 }, axisTick: { show: false } },
      yAxis: { type: 'value', name: '平均得分率', nameTextStyle: { color: '#b0b8c8', fontSize: 10 }, axisLabel: { color: '#b0b8c8', fontSize: 9, formatter: '{value}%' }, splitLine: { lineStyle: { color: 'rgba(255,255,255,0.04)' } }, min: 80, max: 100 },
      series: [{
        type: 'bar',
        id: 'total',
        barWidth: '40%',
        ...
        label: { show: true, position: 'top', color: '#eef0f4', fontSize: 14, fontWeight: 'bold', formatter: '{c}%' },
      }],
    };
      ...
      front={
        <div ref={scrollRef} className="card-border glow-blue p-4 relative overview-chart-card">
          ...
          <p className="text-xs text-text-muted text-center mt-1">
            数据来源：score_distribution 真实分数字段（855所学校）· 每4秒形态切换
          </p>
        </div>
      }
