...
const categoryColors: Record<string, { main: string; light: string }> = {
  'A类-学校管理与安全': { main: '#4da8ff', light: '#4da8ff88' },
  'B类-办学硬件与环境': { main: '#00d4ff', light: '#00d4ff88' },
  'C类-师资队伍与发展': { main: '#a855f7', light: '#a855f788' },
};

function getColor(avgRate: number, baseColor: string): string {
  // Worse rates → brighter (alert) colors; good rates → subtle colors
  if (avgRate < 0.5) return '#ff5c5c';
  if (avgRate < 0.7) return '#ff9f43';
  if (avgRate < 0.85) return '#facc15';
  if (avgRate < 0.95) return '#4ade80';
  return baseColor;
}
...
export default function IndicatorSunburst({ data }: { data: DashboardData }) {
  ...
  useEffect(() => {
    ...
    const rateColor = (rate: number): string => {
      if (rate < 0.5) return '#ff5c5c';
      if (rate < 0.7) return '#ff9f43';
      if (rate < 0.85) return '#facc15';
    ...
    for (const [cat, items] of Object.entries(grouped)) {
      ...
      const subChildren = Object.entries(subGroups).map(([sg, sgItems]) => ({
        name: sg,
        itemStyle: { color: catColor, borderColor: 'rgba(255,255,255,0.15)', borderWidth: 1 },
        children: sgItems.map(ind => {
          ...
          return {
            ...
            itemStyle: { color, borderColor: '#0a0e1a', borderWidth: 1 },
            label: { color },
          };
        }),
      }));
      ...
      treeChildren.push({
        name: catName,
        itemStyle: { color: catColor, borderColor: 'rgba(255,255,255,0.2)', borderWidth: 2 },
    ...
    const option: echarts.EChartsCoreOption = {
      ...
      series: [{
        ...
        lineStyle: { color: 'rgba(255,255,255,0.12)', width: 1.2, curveness: 0.4 },
  ...
  return (
    <FlipCard
      front={
        <div ref={scrollRef} className="card-border glow-cyan p-4 relative overview-chart-card">
