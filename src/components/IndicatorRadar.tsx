...
const categoryMeta: Record<string, { name: string; color: string }> = {
  'A类-学校管理与安全': { name: 'A. 学校管理与安全', color: '#4da8ff' },
  'B类-办学硬件与环境': { name: 'B. 办学硬件与环境', color: '#00d4ff' },
  'C类-师资队伍与发展': { name: 'C. 师资队伍与发展', color: '#a855f7' },
};
...
export default function IndicatorRadar({ data }: { data: DashboardData }) {
  ...
  useEffect(() => {
    ...
    const option: echarts.EChartsCoreOption = {
      ...
      legend: buildSideLegend(['小学', '初中', '九年制'], { fontSize: 11 }),
      radar: {
        ...
        splitArea: {
          areaStyle: { color: ['rgba(77,168,255,0.02)', 'rgba(77,168,255,0.02)'] },
        },
      },
      series: [
        {
          ...
          areaStyle: { color: 'rgba(77,168,255,0.1)' },
  ...
  return (
    <FlipCard
      front={
        <div ref={scrollRef} className="card-border glow-purple p-4 relative overview-chart-card" style={{ '--tw-glow-color': 'rgba(168,85,247,0.1)' } as React.CSSProperties}>
          ...
          <p className="text-xs text-text-muted text-center mt-1 overview-chart-footer">数值越靠近外圈表示得分率越高（满分=1.0）</p>
        </div>
      }
