...
const tabs: { id: TabId; label: string; icon: string; color: string }[] = [
  { id: 'overview', label: '总体概览', icon: '📊', color: '#4da8ff' },
  { id: 'regional', label: '区域分析', icon: '🌍', color: '#00d4ff' },
  { id: 'safety', label: '安全管理', icon: '🛡️', color: '#4ade80' },
  { id: 'facility', label: '硬件设施', icon: '🏗️', color: '#ff9f43' },
  { id: 'faculty', label: '师资队伍', icon: '👩‍🏫', color: '#a855f7' },
];
...
export default function NavigationBar({
  ...
  return (
    <nav className={`sticky top-0 z-50 w-full backdrop-blur-xl bg-bg-primary/80 border-b border-border-subtle ${compact ? 'nav-compact' : ''}`}>
      <div className={`w-full px-4 flex justify-center items-center ${compact ? 'py-2' : 'py-3'}`}>
                ...
                style={
                  isActive
                    ? {
                        ...
                        boxShadow: `0 0 20px ${tab.color}20, inset 0 1px 0 ${tab.color}30`,
