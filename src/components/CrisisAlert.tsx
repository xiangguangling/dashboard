...
export default function CrisisAlert({ data }: { data: DashboardData }) {
  ...
  useEffect(() => {
    if (!inView || hasRendered.current || !gaugeRef.current || !pieRef.current) return;
      ...
      series: [
        {
          ...
          axisLabel: {
            color: '#9aa0b0', fontSize: 14, distance: -50, rotate: 'tangential',
            formatter: (v: number) => {
              if (v === 87.5) return '优秀';
              if (v === 62.5) return '良好';
              if (v === 37.5) return '一般';
          ...
          title: { offsetCenter: [0, '-10%'], color: '#9aa0b0', fontSize: 16 },
          detail: {
      ...
      series: [
        {
          ...
          label: {
            ...
            rich: {
              total: { fontSize: 32, fontWeight: 'bold', color: '#e8eaed' },
              label: { fontSize: 12, color: '#9aa0b0', padding: [8, 0, 0, 0] },
            },
          },
          emphasis: {
  ...
  return (
    <FlipCard
      front={
        <div ref={scrollRef} className="card-border glow-red p-4 relative overview-chart-card">
