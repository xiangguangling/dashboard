...
export default function UrbanRuralHeatmap({ data }: { data: DashboardData }) {
  ...
  useEffect(() => {
    ...
    const option: echarts.EChartsCoreOption = {
      ...
      xAxis: {
        type: 'category',
        data: xLabels,
        axisLabel: { color: '#e8eaed', fontSize: 12, fontWeight: 'bold', margin: 10 },
        axisTick: { show: false },
      ...
      series: [
        {
          type: 'heatmap',
          data: heatData,
          label: buildHeatmapLabelStyle(11),
          emphasis: {
            itemStyle: {
              shadowBlur: 10,
    ...
    <FlipCard
      front={
        <div ref={scrollRef} className="card-border glow-cyan p-4 relative overview-chart-card">
          ...
          <p className="text-xs text-text-muted text-center mt-1 overview-chart-footer">颜色越深表示得分率越高，揭示城乡学校资源配置差异</p>
        </div>
      }
